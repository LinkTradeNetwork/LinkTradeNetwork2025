...
<div class="wrap">
  ...
  <!-- LIVE MEMBER COUNTER -->
  <div style="display:flex;align-items:center;gap:12px;background:rgba(255,255,255,0.2);backdrop-filter:blur(10px);border:1px solid rgba(255,255,255,0.35);border-radius:14px;padding:12px 20px;margin-bottom:16px;max-width:480px;width:100%;animation:fadeIn 0.8s ease-out 0.15s both">
    <div style="display:flex;align-items:center;gap:6px">
      <div style="width:8px;height:8px;border-radius:50%;background:#22c55e;box-shadow:0 0 0 3px rgba(34,197,94,0.3);animation:pulse 2s infinite"></div>
      <span style="font-size:12px;font-weight:600;color:rgba(255,255,255,0.9)">LIVE</span>
    </div>
    <div style="flex:1;text-align:center">
      <span style="font-size:15px;font-weight:700;color:white">
        <span id="member-count" style="font-size:22px;font-weight:800;color:white">...</span>
        trade professionals have joined LTN
      </span>
    </div>
  ...
  <div class="card">
    ...
    <div style="
      ...
      border:1px solid rgba(234,106,0,0.25);
      border-radius:12px;
      padding:12px 16px;
      margin:0 0 18px;
      text-align:center;
      ...
      background:#fff7ed;
    ">
      🎥 After you sign up, you can upload a <b>10-second video clip</b> to showcase your work and get discovered on LinkTradeNetwork.
    </div>
    ...
    <div id="joinStep">
      ...
      <div class="referral-box">
        <label>🔗 Referral Code <span style="font-size:11px;font-weight:500;color:var(--muted)">(optional)</span></label>
        <input
        ...
        />
        <div class="referral-hint">Optional — if someone gave you a code, enter it here. Leave blank if you do not have one.</div>
      </div>
      <!-- ===== END REFERRAL CODE FIELD ===== -->
      ...
      <div id="passwordStep" style="display:none;margin-top:20px;padding-top:20px;border-top:2px solid var(--line)">
        <div style="text-align:center;margin-bottom:16px">
          <div style="font-size:28px;margin-bottom:8px">🔐</div>
          <div style="font-weight:700;font-size:18px;color:var(--ink)">Create Your Password</div>
          <div style="font-size:13px;color:var(--muted);margin-top:4px">You will use this to sign in next time</div>
        </div>
        <label>Password</label>
        <input id="newPassword" type="password" placeholder="Minimum 6 characters" autocomplete="new-password" />
        <label>Confirm Password</label>
        <input id="confirmPassword" type="password" placeholder="Repeat your password" autocomplete="new-password" />
        <div style="font-size:12px;color:var(--muted);margin-top:8px">
          • At least 6 characters
        </div>
        <button id="setPasswordBtn">Save Password & Enter LTN</button>
    ...
    <div id="signinStep" style="display:none;">
      ...
      <label>Password</label>
      <input id="signinPassword" type="password" placeholder="Your password" autocomplete="current-password" />
      <button id="signinBtn">Sign In</button>
      <div style="text-align:center;margin-top:12px">
        <span style="font-size:13px;color:var(--muted)">Forgot your password? </span>
        <a href="#" id="forgotPasswordLink" style="font-size:13px;color:var(--brand);font-weight:600">Reset it here</a>
      </div>
      <div id="signinMsg" class="msg"></div>

      <!-- FORGOT PASSWORD FLOW -->
      <div id="forgotStep" style="display:none;margin-top:20px;padding-top:20px;border-top:2px solid var(--line)">
        <div style="text-align:center;font-size:14px;color:var(--muted);margin-bottom:12px">
          Enter your email — we will send a reset code
        </div>
        <label>Email Address</label>
        <input id="resetEmail" type="email" placeholder="you@example.com" autocomplete="email" />
        <button id="sendResetBtn">Send Reset Code</button>
        <div id="resetStep2" style="display:none;margin-top:16px">
          <div style="text-align:center;font-size:13px;color:var(--muted);margin-bottom:8px">Enter the code sent to your email</div>
          ...
          <input id="resetNewPassword" type="password" placeholder="Min 8 characters with a number" />
          <label>Confirm New Password</label>
          <input id="resetConfirmPassword" type="password" placeholder="Repeat your password" />
...
<script>
// ===== API CONFIG =====
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyJpONCk3k-GF3wfXAdqxNHQELDzLNHj-HhhECj1-xVrEgG6ettGRNdmUXjRj8F4cbH/exec";
...
const firstNameInput=$('#firstName'), lastNameInput=$('#lastName');
const emailInput=$('#email'), consent=$('#consent');
...
let currentEmail='', currentFirstName='', currentLastName='', currentRefCode='';
...
verifyBtn.onclick=async()=>{
  const c=code();
  if(c.length!==6){ showMsg(otpMsg,'Please enter the complete 6-digit code','err'); return; }
  verifyBtn.disabled=true; verifyBtn.textContent='Verifying...';
  try{
    const r=await postJSON({ action:'verify', email:currentEmail, code:c });
    if(r.success){
      const userName=(r.user&&r.user.name)||`${currentFirstName} ${currentLastName}`;
      localStorage.setItem('ltn_user',JSON.stringify({firstName:currentFirstName,lastName:currentLastName,name:userName,email:currentEmail}));
      ...
      if(r.needsPassword){
        ...
        doneStep.style.display='block'; doneStep.classList.add('show');
        setTimeout(()=>{ window.location.replace('front.html'); },1200);
      }
    } else { throw new Error(r.error||'Invalid or expired code'); }
  } catch(e){
    showMsg(otpMsg,e.message,'err');
    codeInputs.forEach(inp=>inp.value=''); codeInputs[0].focus();
  }
...
document.getElementById('setPasswordBtn').onclick=async()=>{
  ...
  if(!pw||!pw2){ showMsg(msg,'Please fill in both fields','err'); return; }
  if(pw!==pw2){ showMsg(msg,'Passwords do not match','err'); return; }
  if(pw.length<6){ showMsg(msg,'Password must be at least 6 characters','err'); return; }
  if(pw!==pw2){ showMsg(msg,'Passwords do not match','err'); return; }
  btn.disabled=true; btn.textContent='Saving...';
  try{
    const r=await postJSON({ action:'set-password', email:currentEmail, password:pw });
    if(r.success){
      showMsg(msg,'✅ Password saved! Entering LTN...','ok');
      setTimeout(()=>{ window.location.replace('front.html'); },1200);
    } else { throw new Error(r.error||'Could not save password'); }
  } catch(e){ showMsg(msg,e.message,'err'); }
  btn.disabled=false; btn.textContent='Save Password & Enter LTN';
};
...
signinBtn.onclick=async()=>{
  ...
  const password=($('#signinPassword')||{value:''}).value.trim();
  if(!email){ showMsg(signinMsg,'Please enter your email address','err'); return; }
  ...
  try{
    const r=await postJSON({ action:'signin', email, password, userAgent:navigator.userAgent });
    if(r.success){
      const userName=(r.user&&r.user.name)||'';
      localStorage.setItem('ltn_user',JSON.stringify({email,name:userName}));
      localStorage.setItem('userEmail',email);
      localStorage.setItem('userName',userName);
      showMsg(signinMsg,'Success! Redirecting...','ok');
      setTimeout(()=>{ window.location.replace('front.html'); },800);
    } else {
      if(r.requiresOtp){
        tabSignup.click();
        showMsg(joinMsg,r.error||'Please sign up to receive a verification code','err');
        emailInput.value=email; firstNameInput.focus();
      } else {
        showMsg(signinMsg,r.error||'Unable to sign in','err');
      }
    }
  } catch(e){ showMsg(signinMsg,'Connection error. Please try again.','err'); }
  signinBtn.disabled=false; signinBtn.textContent='Sign In';
};

// Forgot password link
const forgotLink=document.getElementById('forgotPasswordLink');
if(forgotLink) forgotLink.onclick=function(e){
  e.preventDefault();
  document.getElementById('forgotStep').style.display='block';
  const fe=document.getElementById('resetEmail');
  if(fe){ fe.value=signinEmail.value||''; fe.focus(); }
};

// Send reset code
const sendResetBtn=document.getElementById('sendResetBtn');
if(sendResetBtn) sendResetBtn.onclick=async()=>{
  const email=(document.getElementById('resetEmail').value||'').trim().toLowerCase();
  const msg=document.getElementById('resetMsg');
  if(!email){ showMsg(msg,'Please enter your email','err'); return; }
  sendResetBtn.disabled=true; sendResetBtn.textContent='Sending...';
  try{
    const r=await postJSON({ action:'forgot-password', email });
    if(r.success){
      showMsg(msg,'Reset code sent! Check your email.','ok');
      document.getElementById('resetStep2').style.display='block';
      const ri=Array.from(document.querySelectorAll('#resetOtpInputs input'));
      ri.forEach((inp,i)=>{
        inp.addEventListener('input',()=>{ inp.value=inp.value.replace(/\D/g,''); if(inp.value&&i<5) ri[i+1].focus(); });
        inp.addEventListener('keydown',ev=>{ if(ev.key==='Backspace'&&!inp.value&&i>0) ri[i-1].focus(); });
      });
    } else { showMsg(msg,r.error||'Could not send reset code','err'); }
  } catch(e){ showMsg(msg,'Connection error','err'); }
  sendResetBtn.disabled=false; sendResetBtn.textContent='Send Reset Code';
};
...
if(resetPasswordBtn) resetPasswordBtn.onclick=async()=>{
  const email=(document.getElementById('resetEmail').value||'').trim().toLowerCase();
  const ri=Array.from(document.querySelectorAll('#resetOtpInputs input'));
  const resetCode=ri.map(i=>i.value).join('');
  const pw=(document.getElementById('resetNewPassword').value||'').trim();
  const pw2=(document.getElementById('resetConfirmPassword').value||'').trim();
  const msg=document.getElementById('resetMsg');
  if(resetCode.length!==6){ showMsg(msg,'Please enter the 6-digit reset code','err'); return; }
  if(!pw||!pw2){ showMsg(msg,'Please fill in both password fields','err'); return; }
  if(pw!==pw2){ showMsg(msg,'Passwords do not match','err'); return; }
  if(pw.length<6){ showMsg(msg,'Password must be at least 6 characters','err'); return; }
  resetPasswordBtn.disabled=true; resetPasswordBtn.textContent='Resetting...';
  try{
    const r=await postJSON({ action:'reset-password', email, code:resetCode, password:pw });
    if(r.success){
      showMsg(msg,'✅ Password reset! You can now sign in.','ok');
      setTimeout(()=>{
        document.getElementById('forgotStep').style.display='none';
        signinEmail.value=email;
        const sp=document.getElementById('signinPassword');
        if(sp){ sp.value=''; sp.focus(); }
      },2000);
    } else { showMsg(msg,r.error||'Could not reset password','err'); }
  } catch(e){ showMsg(msg,'Connection error','err'); }
  resetPasswordBtn.disabled=false; resetPasswordBtn.textContent='Reset Password';
};

// Enter key handlers
firstNameInput.addEventListener('keypress', e => { if(e.key==='Enter') lastNameInput.focus(); });
lastNameInput.addEventListener('keypress',  e => { if(e.key==='Enter') emailInput.focus(); });
emailInput.addEventListener('keypress',     e => { if(e.key==='Enter') joinBtn.click(); });
signinEmail.addEventListener('keypress',    e => { if(e.key==='Enter') signinBtn.click(); });
...
(function(){
  ...
  if(ref && referralCodeInput){
    referralCodeInput.value = ref.toUpperCase().replace(/[^A-Z0-9]/g,'');
  }
})();
...
(function(){
  ...
  async function fetchMemberCount() {
    ...
    try {
      const res = await fetch(APPS_SCRIPT_URL, {
        method: 'POST',
        body: JSON.stringify({ action: 'get-member-count' }),
        redirect: 'follow'
      });
      const text = await res.text();
